...
export default function BridgeStation({
  ...
  const [fullscreenPanel, setFullscreenPanel] = useState<'none' | 'ecdis' | 'radar'>('none');
      ...
      {fullscreenPanel === 'none' ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 flex flex-col gap-2">
            <div className="flex items-center justify-between bg-slate-900/60 px-4 py-2 rounded-xl border border-slate-800/80 shadow-md">
              <div className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 bg-cyan-400 rounded-full animate-pulse"></span>
                <span className="text-[10px] font-mono font-bold text-slate-300 uppercase tracking-widest">
                  ECDIS Primary Console
                </span>
              </div>
              <button
                type="button"
                onClick={() => setFullscreenPanel('ecdis')}
                className="bg-slate-950 hover:bg-slate-900 text-cyan-400 hover:text-cyan-300 border border-cyan-800/40 hover:border-cyan-400 px-3 py-1 rounded-lg text-[10px] font-bold tracking-wider uppercase transition flex items-center gap-1.5 cursor-pointer shadow-sm active:scale-95 shrink-0"
                title="Fullscreen ECDIS Map Terminal"
              >
                <span>Maximize ECDIS ⛶</span>
              </button>
            </div>
            <MapChart
              waypoints={waypoints}
          ...
          <div className="lg:col-span-1 flex flex-col gap-2">
            <div className="flex items-center justify-between bg-slate-900/60 px-4 py-2 rounded-xl border border-slate-800/80 shadow-md">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-mono font-bold text-slate-300 uppercase tracking-widest">
                  Tactical Range Monitor
                </span>
              </div>
              <button
                type="button"
                onClick={() => setFullscreenPanel('radar')}
                className="bg-slate-950 hover:bg-slate-900 text-cyan-400 hover:text-cyan-300 border border-cyan-800/40 hover:border-cyan-400 px-3 py-1 rounded-lg text-[10px] font-bold tracking-wider uppercase transition flex items-center gap-1.5 cursor-pointer shadow-sm active:scale-95 shrink-0"
                title="Fullscreen Radar Scope"
              >
                <span>Maximize Radar ⛶</span>
              </button>
            </div>
            <RadarScreen />
          </div>
        </div>
      ) : fullscreenPanel === 'ecdis' ? (
        <div className="space-y-3.5 animate-fade-in" id="full-ecdis-container">
          <div className="flex items-center justify-between bg-slate-900/90 border border-cyan-500/30 p-3 rounded-xl shadow-2xl">
            <div className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 bg-emerald-400 rounded-full animate-pulse"></span>
              <span className="text-xs font-mono font-bold text-slate-200 uppercase tracking-widest text-glow-cyan">ECDIS Fullscreen Navigation Map Watch</span>
            </div>
            <button
              type="button"
              onClick={() => setFullscreenPanel('none')}
              className="bg-rose-950/80 hover:bg-rose-900 text-rose-300 border border-rose-800 px-4 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider transition active:scale-95 cursor-pointer shadow-lg"
            >
              ✕ Exit Fullscreen [ESC]
            </button>
          </div>
          <div className="relative min-h-[550px] h-[75vh]">
            <MapChart
              waypoints={waypoints}
              currentWaypointIndex={currentWaypointIndex}
              voyageStarted={voyageStarted}
        ...
        <div className="space-y-3.5 animate-fade-in" id="full-radar-container">
          <div className="flex items-center justify-between bg-slate-900/90 border border-cyan-500/30 p-3 rounded-xl shadow-2xl">
            <div className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 bg-emerald-400 rounded-full animate-pulse"></span>
              <span className="text-xs font-mono font-bold text-slate-200 uppercase tracking-widest text-glow-cyan">Tactical Radar Screen Fullscreen Active Watch</span>
            </div>
            <button
              type="button"
              onClick={() => setFullscreenPanel('none')}
