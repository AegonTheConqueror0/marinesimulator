...
export default function App() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (engine.state === 'running') {
        ...
        // Temp target depends on RPM, starting at stable preheated 80°C
        const targetTemp = 80 + Math.round((nextEngine.rpm / 800) * 12);
        const sensorSpiked = activeAlarms.some(a => a.system === 'engine' && !a.acknowledged);
        const currentTargetTemp = sensorSpiked ? 114 : targetTemp;

        if (Math.abs(engine.temp - currentTargetTemp) >= 0.2) {
          nextEngine.temp = parseFloat((engine.temp + (engine.temp < currentTargetTemp ? 0.4 : -0.4)).toFixed(1));
        } else {
        ...
        // Slowly return jacket temperature back to preheated 80°C from running temperature
        if (Math.abs(engine.temp - 80) >= 0.2) {
          nextEngine.temp = parseFloat((engine.temp + (engine.temp < 80 ? 0.4 : -0.4)).toFixed(1));
        } else {
          nextEngine.temp = 80;
        }
  ...
  const handleSendMessage = useCallback((sender: 'bridge' | 'engine' | 'admin' | 'remote', receiver: 'bridge' | 'engine' | 'admin' | 'remote' | 'all', content: string) => {
  ...
  return (
    <div className="min-h-screen bg-marine-950 text-slate-300 font-sans selection:bg-cyan-500 selection:text-slate-900 pb-12 maritime-grid">
      ...
      {/* Academy Roleplay companion widget floating bottom-right */}
      <RoleplayScriptWidget 
        onTransmitMessage={handleSendMessage}
